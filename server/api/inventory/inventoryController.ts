...
export const inventory_list = asyncHandler(async (req, res, next) => {
    ...
    const store1 = {
        availProducts: 
            inventory.rows.filter((row :{inventory_store_1: number}) => 
                row.inventory_store_1
            ).length,
        inventory: 
                inventory.rows.map((row :{product_name: string, inventory_store_1: number, product_id: number}) => {
    ...
    const store2 = {
        availProducts: 
            inventory.rows.filter((row :{inventory_store_2: number}) => 
                row.inventory_store_2
            ).length,
        inventory: 
                inventory.rows.map((row :{product_name: string, inventory_store_2: number, product_id: number}) => {
    ...
    const store3 = {
        availProducts: 
            inventory.rows.filter((row :{inventory_store_3: number}) => 
                row.inventory_store_3
            ).length,
        inventory: 
                inventory.rows.map((row :{product_name: string, inventory_store_3: number, product_id: number}) => {
