...
import { useEditSaleMutation, useGetSaleQuery } from '../../features/sales/salesApiSlice'
...
const EditSalePage = () => {
    ...
    const [editSale, {isLoading: editIsLoading}] = useEditSaleMutation()
    ...
    const handleSubmit = async (e) => {
        ...
        if (!editIsLoading) {
            try {
                const response = await editSale(formData)
                navigate(-1)
            } catch (err) {
                console.error(err)
            }
    ...
    useEffect(() => {
        if (sale) {
            setProducts(prevState => ({
                ...prevState,
                allSelected: sale.sale.products.map(product => {
                    const p = {
            ...
            setCustomers(prevState => ({
                ...
                selected: {
                    id: sale.customer.id,
                    name: sale.customer.firstName.concat(' ', sale.customer.lastName),
            ...
            setStaff(sale.staff)
            setDate(sale.date)
            setPaymentMethod(sale.sale.paymentMethod)
        }
    }, [sale])
    ...
    if (saleIsLoading || productsApiIsLoading || customersApiIsLoading) content = <Spinner />
    if (saleIsSuccess && productsApiIsSuccess && customersApiIsSuccess) {
                ...
                case 'product': {
                    ...
                    setProducts(prevState => ({
                        ...
                        currSelected: {
                            ...
                            price: Number(productsApi.entities[id].price)
                        }
                    }))
                    break
                }
